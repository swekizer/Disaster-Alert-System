...
async function runDisasterEngine() {
  console.log('[Engine] Running scheduled data sync for Open-Meteo & NASA FIRMS...');
  
  try {
    const openMeteoEvents = await fetchOpenMeteoExtremeWeather();
    const firmsEvents = await fetchFirmsActiveFires();
    
    const allEvents = [...openMeteoEvents, ...firmsEvents];
    let upsertCount = 0;
    ...
    console.log(`[Engine] Successfully saved ${upsertCount} combined events to the database.`);
  } catch (error) {
    console.error('[Engine] Error running disaster engine fetch', error.message);
  }
}
