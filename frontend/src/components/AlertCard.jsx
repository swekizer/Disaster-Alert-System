  ...
  return (
    <div className={`alert-card severity-${severity}`}>
      ...
      <div className={`alert-icon ${disasterType} default`}>
        <span role="img" aria-label={disasterType}>{icon}</span>
      </div>

      {/* Body */}
      <div className="alert-body">
        <div className="alert-title" title={title}>{title}</div>
        ...
        <div className="alert-meta">
          ...
          {magnitude != null && (
            <span className="badge badge-medium" style={{ fontSize: '0.68rem', padding: '1px 7px' }}>
              M{magnitude}
            </span>
          )}
          <span className={`alert-source source-${sourceAPI}`}>{sourceAPI}</span>
        </div>
      </div>
